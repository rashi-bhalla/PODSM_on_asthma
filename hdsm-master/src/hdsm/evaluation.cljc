...
(defn results-summary
  ([label batch1 site-structure results site-changelog
   ...
   (let [sites (if (map? site-structure)
	...
	site-communication-algo (get-communication-to-sites algo-results (keys sites))
         ...
         total-comm (if (clojure.string/includes? label ":asthma-centralised") 1 (total-site-communication-algo site-communication-algo sites))
         full-comm (if (clojure.string/includes? label ":asthma-centralised") 12 (get-full-transmission site-communication-algo))
         full (if (clojure.string/includes? label ":asthma-centralised") 12 (get-full-transmission site-communication))
         site-hits (->> results
                        (map :breakdown)
                        (flatten)
                        (map keys)
...
(println "total" total-comm)
     {:results-count results-count
      ...
      :proportion-transmitted (percent (if (clojure.string/includes? label ":asthma-centralised") 1 (get-proportion-transmitted weighted-site-communication)))
 ...
 :algo-accuracy (percent (get-accuracy algo-results))
     :total-site-communication-algo (total-site-communication-algo site-communication-algo sites)
       :full-comm-algo (if (clojure.string/includes? label ":asthma-centralised") 12 (get-full-transmission site-communication-algo))
	   :full (if (clojure.string/includes? label ":asthma-centralised") 12 (get-full-transmission site-communication))
      :subset-summaries
      (zipmap skip-records
              (for [skip-count skip-records]
                (results-summary label batch1 site-structure
